...
def main():
    try:
        aa_alignment_path = sys.argv[1]
        codon_path = sys.argv[2]
        output_directory = sys.argv[3]
    except IndexError:
        print("Usage: aa2codon.py <Amino Acid Fasta Alignment> <Nucleotide Fasta> <Output Directory> \n"
              "Converts amino acid sequences into their respective codon sequences uses Codon Table 1 \n")
        return
    #reference_genome = sys.argv[4]
    clusters = {}
    aa_alignment = readFastaFile(aa_alignment_path, alphabet=Protein_Alphabet_wGAPX)
    codons = readFastaFile(codon_path, alphabet=DNA_Alphabet_wGAPN)
    for aa_seq in aa_alignment:
        for nuc_seq in codons:
            if nuc_seq.name == aa_seq.name:
                new_seq = ""
                window_start, window_end = 0, 3
                for aa in aa_seq.sequence:
                    if aa == "-":
                        new_seq += "---"
                    else:
                        new_seq += nuc_seq.sequence[window_start:window_end]
                        window_start, window_end = window_start + 3, window_end + 3
                nuc_seq.sequence = new_seq
    ...
    new_name = codon_path.split('/')[-1]
    writeFastaFile(output_directory+'/'+new_name, codons)
...
DNA_Alphabet_wGAP = Alphabet('ACGT-')
DNA_Alphabet_wGAPN = Alphabet('ACGTN-')
...
Protein_Alphabet_wGAP  = Alphabet('ACDEFGHIKLMNPQRSTVWY-')
Protein_Alphabet_wGAPX  = Alphabet('ACDEFGHIKLMNPQRSTVWYX-')
DSSP_Alphabet = Alphabet('GHITEBSC')
DSSP3_Alphabet = Alphabet('HEC')

predefAlphabets = {'Bool_Alphabet': Bool_Alphabet,
                   'DNA': DNA_Alphabet,
                   'DNAwGAP': DNA_Alphabet_wGAP,
                   'DNAwGAPN': DNA_Alphabet_wGAPN,
                   'RNA': RNA_Alphabet,
                   ...
                   'ProteinwGAP': Protein_Alphabet_wGAP,
                   'ProteinwGAPX': Protein_Alphabet_wGAPX,
